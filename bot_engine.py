...
# --- 🔥 NEW: PM & PRESENCE LISTENER 🔥 ---
                ...
                self.log(f"👤 Presence Update: {user} is {mode} | Status: '{status}'")
...
# --- 🔥 NEW: PRIVATE MESSAGE (PM) ACTIONS 🔥 ---
    def send_pm_message(self, to_user, text):
        self.send_json({"handler": "chat_message", "id": uuid.uuid4().hex, "to": to_user, "type": "text", "body": text})

    def send_pm_image(self, to_user, url):
        self.send_json({"handler": "chat_message", "id": uuid.uuid4().hex, "to": to_user, "type": "image", "url": url, "body": ""})

    def send_pm_audio(self, to_user, url):
        self.send_json({"handler": "chat_message", "id": uuid.uuid4().hex, "to": to_user, "type": "audio", "url": url, "body": ""})
    
    # --- 🔥 NEW: PRESENCE ACTION (Status Update) 🔥 ---
